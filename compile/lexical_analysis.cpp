...
int main()
{
  ...
  // 变量名表
  char vname;  // 变量名vname: char(16)
  char vproc;  // 所属过程vproc:char(16)
  int vkind;   // 分类vkind: 0..1(0—变量 1—形参)
  char vtypes; // 变量类型vtype: types
  int vlev;    // 变量层次vlev: int
  int vadr;    // 变量在变量表中的位置vadr: int(相对第一个变量而言)

  // 过程名表
  char pname;    // 过程名pname: char(16)
  char ptype;    // 过程类型ptype: types
  int plev;      // 过程层次plev: int
  int pfirstadr; // 第一个变量在变量表中的位置fadr: int
  int plastadr;  // 最后一个变量在变量表中的位置ladr: int
  ifstream infile;
  ofstream outfile;
